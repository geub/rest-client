...
public class HTTPClientRequestExecuter implements RequestExecuter {
	...
	public void execute(Request request, View... views) {
		...
		ProtocolVersion protocolVersion =
			...
			try {
				...
				HttpResponse http_res = this.httpClient.execute(req, httpContext);
					...
					if(entity != null) {
						if(request.isIgnoreResponseBody()) {
							...
							try{
								...
								HTTPClientRequestExecuter.LOG.log(Level.WARNING, ex.getMessage(), ex);
							}
						}
					}
				...
				if (!this.interruptedShutdown) {
					...
					try {
						...
						HTTPClientRequestExecuter.LOG.log(Level.WARNING, "Exception when closing httpClient", ex);
					}
				}
				else {
	...
	public void abortExecution(){
		if(!this.isRequestCompleted){
			...
			HTTPClientRequestExecuter.LOG.info("Request already completed. Doing nothing.");
		}
	}
}
